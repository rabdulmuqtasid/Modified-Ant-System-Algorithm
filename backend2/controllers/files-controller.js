...
const getFileById = async (req, res, next) => {
  ...
  let files;
  try {
    files = await File.findById(userId);
  } catch (err) {
  ...
  res.json({ files: files.toObject({ getters: true }) });
};
...
const getFilesByUserId = async (req, res, next) => {
  ...
  res.json({
    // files: userWithFiles.files.map(filess =>
    //   filess.toObject({ getters: true })
    // )
    userWithFiles: userWithFiles.toObject({ getters: true })
  });
};
